...
int main(int argc, char* argv[]) {
  ...
  auto* model_list = model_cmd->add_subcommand("list", "List installed (and optionally remote) models");
  model_list->add_flag("--remote", model_list_remote, "Include remote models from registry");

  std::string model_add_name;
  auto* model_add = model_cmd->add_subcommand("add", "Download and install a model");
  ...
  auto* model_remove = model_cmd->add_subcommand("remove", "Remove an installed model");
  ...
  scene_remove->add_flag("--force", scene_remove_force, "Force removal of built-in scenes");

  // ---- llm subcommand ----
  auto* llm_cmd = app.add_subcommand("llm", "Manage LLM providers");
  llm_cmd->require_subcommand(1);

  auto* llm_list = llm_cmd->add_subcommand("list", "List configured LLM providers");
  ...
  auto* llm_remove = llm_cmd->add_subcommand("remove", "Remove an LLM provider");
  ...
  // ---- config subcommand ----
  auto* config_cmd = app.add_subcommand("config", "Read or write configuration values");
  config_cmd->require_subcommand(1);

  std::string config_get_path;
  auto* config_get = config_cmd->add_subcommand("get", "Get a config value by dotpath");
  config_get->add_option("path", config_get_path, "Config dotpath (e.g. fcitx.triggerKey)")->required();

  std::string config_set_path;
  std::string config_set_value;
  bool config_set_stdin = false;
  auto* config_set = config_cmd->add_subcommand("set", "Set a config value by dotpath");
  ...
  auto* config_edit = config_cmd->add_subcommand("edit", "Open config file in editor");
  config_edit->add_option("target", config_edit_target, "Config target: fcitx or extra")->required();
  ...
  auto* daemon_restart = daemon_cmd->add_subcommand("restart", "Restart the daemon");
  ...
  daemon_logs->add_option("-n", daemon_logs_lines, "Number of lines to show")->default_val(20);

  // ---- init subcommand ----
  auto* init_cmd = app.add_subcommand("init", "Initialize default config and directories");
  ...
  // scene
    ...
    return RunSceneAdd(scene_add_id, scene_add_label,
  ...
  // llm
    ...
    return RunLlmAdd(llm_add_name, llm_add_base_url, llm_add_model, llm_add_api_key, *fmt, ctx);
  ...
  // config
    ...
    return RunConfigSet(config_set_path, config_set_value, config_set_stdin, *fmt, ctx);
